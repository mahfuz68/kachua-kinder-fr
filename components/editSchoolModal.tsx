...
export default function EditSchoolModal({ prop }: Props) {
  ...
  const updateSchool = async (name: string) => {
    try {
      setLoading(true);

      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/school/${prop?.school?.id}`,
        {
          method: "PATCH",
          body: JSON.stringify({
            eiin: Number(convertToEngNumber(name)),
            examCenterId: prop?.school?.examCenterId,
          }),
          headers: {
  ...
  return (
    <div
      ...
      aria-hidden="false"
      className={`${
        prop.openModal === prop?.school?.id ? "block" : "hidden"
          ...
          <div>
            ...
            <div className="mt-6 ml-6 flex gap-x-4 pb-4">
              ...
              >
                ...
                <span className="hidden md:block">স্কুল ইডিট করুন</span>
                <span className="block md:hidden">স্কুল ইডিট</span>
              </button>
              <button
                onClick={() => {
                  setError("");
