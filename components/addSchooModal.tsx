...
export default function AddSchoolModal({ prop }: Props) {
  ...
  const crateUser = async (name: string) => {
    try {
      setLoading(true);
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/school/`, {
        ...
        body: JSON.stringify({
          ...
          examCenterId: prop.examCenterId,
          examId: prop.examId,
        }),
        headers: {
      ...
      if (data.error) {
        ...
        prop.setExamWiseSchool([data, ...prop.examWiseSchool]);
        clearState();
          ...
          <div>
            ...
            <div className="mt-6 ml-6 flex gap-x-4 pb-4">
              ...
              >
                ...
                <span className="hidden md:block">স্কুল অ্যাড করুন</span>
                <span className="block md:hidden">স্কুল অ্যাড</span>
              </button>
              <button
                onClick={() => {
                  prop.setOpenModal(undefined);
