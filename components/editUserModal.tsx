...
export default function UpdateUserModal({ prop }: Props) {
  const { data: session } = useSession();
  const user: any = session?.user;
  const [name, setName] = useState(prop?.user?.name);
  const [mobile, setMobile] = useState(
    englishToBanglaNumber(prop?.user?.mobile)
  );
  const [eye, setEye] = useState(false);
  const [schoolName, setSchoolName] = useState(prop?.user?.profile?.schoolName);
  const [eiin, setEiin] = useState(
    englishToBanglaNumber(prop?.user?.profile?.eiin)
  );
  ...
  const crateUser = async (
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/users/${prop?.user?.id}`,
        {
          method: "PATCH",
      ...
      if (data.error) {
        ...
        prop.sdata([
          ...prop.data.map((usr: any) => {
            if (usr?.id !== prop?.user?.id) {
              return usr;
            } else {
              return data;
            }
  ...
  return (
    <div
      ...
      aria-hidden="false"
      className={`${
        prop.openModal === prop?.user?.id ? "block" : "hidden"
            ...
            <div className="mt-4 ml-6 flex gap-x-4 pb-10 md:pb-4">
              <button
                onClick={(e) => {
                  ...
                  if (name && mobile && schoolName && eiin) {
              ...
              >
                ...
                <span className="hidden md:block">ইউজার ইডিট করুন</span>
                <span className="block md:hidden">ইউজার ইডিট</span>
              </button>
              <button
                onClick={() => {
                  clearState();
